...
def plot_class_distribution(
    df: pd.DataFrame, target_col: str = "behavior", save_path: Optional[Path] = None, figsize: Tuple[int, int] = (12, 5)
) -> plt.Figure:
...
def plot_feature_distributions(
    ...
    for ax, feat in zip(axes[:n_features], features):
        for behavior in ["NORMAL", "DROWSY", "AGGRESSIVE"]:
            ...
            if len(data) > 0:
                ax.hist(data, alpha=0.6, label=behavior, bins=12, color=COLORS[behavior], edgecolor="white")
...
def plot_correlation_matrix(
    df: pd.DataFrame, feature_cols: List[str], save_path: Optional[Path] = None, figsize: Tuple[int, int] = (14, 12)
) -> plt.Figure:
...
def plot_model_comparison(
    results: List[ClassificationResult], save_path: Optional[Path] = None, figsize: Tuple[int, int] = (16, 7)
) -> plt.Figure:
    ...
    # Test accuracy
    colors = plt.cm.RdYlGn(np.array(test_accs))  # type: ignore[attr-defined]
...
def plot_feature_importance(
    ...
    colors = plt.cm.viridis(np.linspace(0.3, 0.9, len(df_sorted)))  # type: ignore[attr-defined]

    bars = ax.barh(
        range(len(df_sorted)), df_sorted["Importance"].values, color=colors, edgecolor="black", linewidth=0.5
    )
...
def plot_behavior_comparison(
    ...
    for i, feature in enumerate(features):
        ...
        sns.boxplot(
            data=df_plot, x=target_col, y=feature, ax=ax, hue=target_col, palette=filtered_palette, legend=False
        )
