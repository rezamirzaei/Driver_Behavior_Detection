...
FEATURE_LINEAGE_CLASSIFICATION: Dict[str, str] = {
    "speed_mean": "RAW_GPS.speed mean over trip samples.",
    "speed_std": "RAW_GPS.speed standard deviation over trip samples.",
    "speed_max": "RAW_GPS.speed maximum observed in trip.",
    "speed_min": "RAW_GPS.speed minimum observed in trip.",
    "speed_range": "RAW_GPS.speed max - min.",
    "speed_q25": "RAW_GPS.speed 25th percentile.",
    "speed_q75": "RAW_GPS.speed 75th percentile.",
    "speed_cv": "RAW_GPS.speed std / mean (coefficient of variation).",
    "speed_change_mean": "Mean absolute first difference of RAW_GPS.speed.",
    "speed_change_std": "Std of absolute first difference of RAW_GPS.speed.",
    "course_change_mean": "Mean absolute first difference of RAW_GPS.course.",
    "course_change_std": "Std of absolute first difference of RAW_GPS.course.",
    "course_change_max": "Max absolute first difference of RAW_GPS.course.",
    "trip_duration": "RAW_GPS.timestamp max - min.",
    "acc_x_mean": "RAW_ACCELEROMETERS.acc_x_kf mean.",
    "acc_x_std": "RAW_ACCELEROMETERS.acc_x_kf std.",
    "acc_x_range": "RAW_ACCELEROMETERS.acc_x_kf max - min.",
    "acc_y_mean": "RAW_ACCELEROMETERS.acc_y_kf mean.",
    "acc_y_std": "RAW_ACCELEROMETERS.acc_y_kf std.",
    "acc_y_range": "RAW_ACCELEROMETERS.acc_y_kf max - min.",
    "acc_z_mean": "RAW_ACCELEROMETERS.acc_z_kf mean.",
    "acc_z_std": "RAW_ACCELEROMETERS.acc_z_kf std.",
    "acc_magnitude_mean": "Mean sqrt(acc_x_kf^2 + acc_y_kf^2 + acc_z_kf^2).",
    "acc_magnitude_std": "Std sqrt(acc_x_kf^2 + acc_y_kf^2 + acc_z_kf^2).",
    "acc_magnitude_max": "Max sqrt(acc_x_kf^2 + acc_y_kf^2 + acc_z_kf^2).",
    "acc_magnitude_q95": "95th percentile sqrt(acc_x_kf^2 + acc_y_kf^2 + acc_z_kf^2).",
    "acc_rms": "Root-mean-square of acceleration magnitude.",
    "jerk_x_mean": "Mean absolute first difference of acc_x_kf.",
    "jerk_x_std": "Std of first difference of acc_x_kf.",
    "jerk_y_mean": "Mean absolute first difference of acc_y_kf.",
    "jerk_y_std": "Std of first difference of acc_y_kf.",
    "jerk_magnitude_std": "Std of sqrt(jerk_x^2 + jerk_y^2).",
    "brake_count": "Count of contiguous events where acc_x_kf < brake threshold.",
    "accel_count": "Count of contiguous events where acc_x_kf > acceleration threshold.",
    "turn_count": "Count of contiguous events where |acc_y_kf| > turn threshold.",
    "sharp_turn_count": "Count of contiguous events where |acc_y_kf| > sharp-turn threshold.",
    "brake_rate": "brake_count / trip_duration.",
    "turn_rate": "turn_count / trip_duration.",
}

FEATURE_LINEAGE_REGRESSION_ENGINEERED: Dict[str, str] = {
    "displ_per_cyl": "displ / max(cylinders, 1).",
    "displ_x_cyl": "displ * cylinders interaction term.",
    "displ_squared": "displ^2 non-linear term.",
    "range_total": "rangeCity + rangeHwy (with missing-value safeguards).",
    "range_city_ratio": "rangeCity / max(range_total, epsilon).",
    "has_supercharger": "Binary flag derived from sCharger string field.",
    "has_turbocharger": "Binary flag derived from tCharger string field.",
    "is_electric": "Binary flag derived from fuel/electric motor indicators.",
    "has_start_stop": "Binary flag derived from startStop field.",
    "is_guzzler": "Binary flag derived from guzzler designation.",
    "is_forced_induction": "Binary OR of supercharger/turbocharger engineered flags.",
    "vehicle_age": "max(year) - year within dataset snapshot.",
}
...
class TaskCatalog(BaseModel):
    ...
    feature_lineage: Dict[str, str] = Field(default_factory=dict)
...
def _fallback_feature_lineage(task: TaskType, feature: str, source: FeatureSourceType) -> str:
    if task == "classification":
        return f"Derived from trip-level aggregation of raw UAH telemetry for '{feature}'."
    if source == "raw":
        return f"Direct value from EPA source column '{feature}'."
    return f"Engineered feature '{feature}' generated by regression preprocessing rules."
...
def _build_feature_lineage(
    ...
    for feature in features:
        if task == "classification":
            lineage[feature] = FEATURE_LINEAGE_CLASSIFICATION.get(
                feature,
                _fallback_feature_lineage(task, feature, feature_sources[feature]),
            )
            continue

        if feature in FEATURE_LINEAGE_REGRESSION_ENGINEERED:
            lineage[feature] = FEATURE_LINEAGE_REGRESSION_ENGINEERED[feature]
            continue

        source = feature_sources[feature]
        lineage[feature] = _fallback_feature_lineage(task, feature, source)
...
def build_task_catalogs(
    ...
    cls_lineage = _build_feature_lineage("classification", cls_features, cls_sources)
    reg_lineage = _build_feature_lineage("regression", reg_features, reg_sources)
